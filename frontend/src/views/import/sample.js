...
export async function createSample(schema, type) {
  ...
  else if(schema.name === "advance_salary") {
      await sampleWithEmployeeRelation(schema, type, "advance_salary_sample_import")
  } 
  else {
...
async function sampleWithEmployeeRelation(schema, type, filename="sample") {
  const data = [];
  data.push(["Note: * field are required"]);
  const headers = [];
  const newRemoveHeader = ["id", "created", "updated"];
